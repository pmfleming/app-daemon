...
CPU, GPU, resident CPU/GPU memory, energy, and battery-equivalent history for active applications is aggregated into 15-second points and retained for 24 hours in `$XDG_STATE_HOME/app-daemon/resource-history-v1.json` (or `~/.local/state/...`). Query it with `applications.history`:
